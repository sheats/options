...
class NASDAQFetcher:
        ...
        if source == "auto":
            ...
            for src in ["ftp", "api", "yahoo"]:
                try:
                    ...
                    if stocks:
                        logger.info(f"Successfully fetched {len(stocks)} stocks from {src}")
                        break
    ...
    def _fetch_from_yahoo(self) -> List[Dict[str, str]]:
        ...
        try:
            ...
            logger.warning("Yahoo Finance direct listing not implemented - use FTP or API source")
            return []
                ...
                with ThreadPoolExecutor(max_workers=10) as executor:
                    ...
                    for future in as_completed(futures):
                        ...
                        if stock_info:
                            ...
                            if min_market_cap is None or stock_info.get('market_cap', 0) >= min_market_cap:
    ...
    def _get_stock_info(self, stock: Dict[str, str]) -> Optional[Dict[str, any]]:
        ...
        try:
            ...
            enhanced_info = {
                'symbol': symbol,
                'name': stock.get('name', info.get('longName', info.get('shortName', symbol))),
    ...
    def _filter_by_market_cap(
        ...
        logger.info(f"Filtering {len(stocks)} stocks by market cap >= ${min_market_cap/1e9:.1f}B...")
        ...
        logger.info(f"Found {len(filtered_stocks)} stocks with market cap >= ${min_market_cap/1e9:.1f}B")
    ...
    def get_nasdaq_500(self, include_info: bool = True) -> List[Dict[str, any]]:
        ...
        if not include_info:
            ...
            nasdaq_500 = [{'symbol': s['symbol'], 'name': s['name']} for s in nasdaq_500]
...
if __name__ == "__main__":
    ...
    if large_caps:
        ...
        for stock in large_caps[:5]:
            print(f"  {stock['symbol']}: {stock['name']} - ${stock['market_cap']/1e9:.1f}B")