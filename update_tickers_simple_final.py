...
def fetch_ticker_data(ticker: str) -> Optional[Dict]:
    ...
    if len(hist) >= 14:
        ...
        rs = gain / loss
        rsi = 100 - (100 / (1 + rs))
        data["rsi_14"] = float(rsi.iloc[-1]) if not rsi.empty else None

        # Calculate MACD
        exp1 = close.ewm(span=12, adjust=False).mean()
        exp2 = close.ewm(span=26, adjust=False).mean()
        macd = exp1 - exp2
        data["macd"] = float(macd.iloc[-1]) if not macd.empty else None
        ...
        if len(hist) > 0:
            ...
            if len(year_ago_hist) > 0:
                data["return_1y"] = float(
                    ((close.iloc[-1] / year_ago_hist["Close"].iloc[0]) - 1) * 100
                )

            # 30 day volatility
            returns = close.pct_change().dropna()
            if len(returns) >= 30:
                vol_30d = returns.rolling(30).std().iloc[-1] * (252**0.5) * 100
                data["volatility_30d"] = float(vol_30d)
    ...
    if earnings is not None and not earnings.empty:
        ...
        earnings_naive = earnings.copy()
        earnings_naive.index = earnings_naive.index.tz_convert(None)
        future_earnings = earnings_naive[earnings_naive.index > now]
...
def update_tickers(
    exchange: str, cache_provider: SQLiteCacheProvider, max_workers: int = 5
):
    ...
    with tqdm(total=len(tickers), desc=f"Updating {exchange}") as pbar:
        with ThreadPoolExecutor(max_workers=max_workers) as executor:
            ...
            for future in as_completed(future_to_ticker):
                ...
                if data:
                    # Save to database with custom JSON encoder
                    with sqlite3.connect(cache_provider.db_path) as conn:
                        cursor = conn.cursor()
                        cursor.execute(
                            """
                ...
                # Rate limiting
                time.sleep(0.5)  # Increased to avoid Yahoo rate limits
...
def generate_hotlist(exchange: str, cache_provider: SQLiteCacheProvider):
    ...
    print(f"{'='*80}")
    print(
        f"{'Ticker':<8} {'Name':<30} {'Score':<8} {'MCap($B)':<10} {'IV Rank':<8} {'Inst%':<8} {'Price':<8}"
    )
    print("-" * 80)

    for stock in hotlist[:10]:
        print(
            f"{stock['ticker']:<8} {stock['name'][:30]:<30} {stock['score']:<8.1f} "
...
def main():
    ...
    parser.add_argument(
        "--workers", type=int, default=2, help="Number of concurrent workers"
    )
    parser.add_argument(
        "--hotlist", action="store_true", help="Generate hotlist after update"
    )
    parser.add_argument(
        "--cache-db",
        type=str,
