...
This is a refactored version using extracted modules for better organization and testability.
"""
...
class CSPScanner:
    ...
    def run_scan(self, tickers: List[str], exchange: str = "SP500") -> pd.DataFrame:
        ...
        if not quality_stocks:
            ...
            pe_limit = constants.NASDAQ_MAX_PE_RATIO if exchange == "NASDAQ" else constants.MAX_PE_RATIO
            logger.info(f"- P/E ratio > {pe_limit}")
            logger.info(f"- Forward EPS <= ${constants.MIN_FORWARD_EPS}")
            return_limit = constants.NASDAQ_MIN_ONE_YEAR_RETURN if exchange == "NASDAQ" else constants.MIN_ONE_YEAR_RETURN
