...
try:
    ...
    logger.warning("tastytrade package not installed. Install with: pip install tastytrade")
...
class TastyTradeProvider(DataProvider):
    ...
    def __init__(self, username: str, password: str):
        if not TASTYTRADE_AVAILABLE:
            raise ImportError("TastyTrade SDK not available. Install with: pip install tastytrade")
            ...
            for option in chains.options:
                if option.option_type == 'P':  # Put options
                    puts_data.append({
                        ...
                        'openInterest': int(option.open_interest) if option.open_interest else 0,
                        'impliedVolatility': float(option.implied_volatility) if option.implied_volatility else 0.0,
