...
class YFinanceProvider(DataProvider):
    ...
    def get_option_chain(self, ticker: str, exp_date: str) -> pd.DataFrame:
        ...
        try:
            ...
            invalid_iv_count = puts[
                pd.isna(puts["impliedVolatility"]) | (puts["impliedVolatility"] <= 0)
            ].shape[0]
            if invalid_iv_count > 0:
                logger.debug(
                    f"{ticker} {exp_date}: {invalid_iv_count} puts with invalid IV will have zero Greeks"
                )

            # Compute Greeks for each option
            for idx, row in puts.iterrows():
                if (
                    pd.notna(row["impliedVolatility"])
